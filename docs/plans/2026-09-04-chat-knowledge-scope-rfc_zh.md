...
| --- | --- |
| 普通发送 | 提交时输入框选择 |
| 编辑后重新生成 | 显式提交编辑时输入框选择则创建新快照；未提交时继承来源轮次已接纳的 scope |
| 普通重新生成 | 服务端原始 HumanMessage 快照，忽略客户端伪造的替换 scope |
| 澄清回复 | 若当前回复显式携带选择器快照，则校验并采用该新 scope；未携带时继承该执行链已接纳的 scope |
| resume | 沿用该执行链已接纳的 scope，不接受客户端替换 |
...
- 普通重新生成用原快照；编辑后重新生成与澄清回复显式携带当前选择时采用新范围，省略时沿用来源轮次的原执行范围。
- 分页选择、加载错误、键盘操作和窄屏可用。
